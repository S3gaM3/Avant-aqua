...
export async function POST(request: Request) {
  ...
  if (hasLeadErrors(errors)) {
    return jsonError(applyRussianNbsp("Проверьте корректность заполнения формы"), 400, { errors });
  }
  ...
  try {
    ...
    return jsonError(applyRussianNbsp("Не удалось отправить заявку. Попробуйте ещё раз."), 500);
  }
}
